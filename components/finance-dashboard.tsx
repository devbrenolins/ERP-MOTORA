...
export function FinanceDashboard({ mode }: { mode: "flow" | "reports" }) {
  ...
  return <main className="mx-auto max-w-[1520px] px-4 py-5 lg:px-7 lg:py-6">
    <div className="mb-5 flex items-start justify-between gap-4"><div><p className="mb-1 text-[11px] font-bold uppercase tracking-[.14em] text-[var(--brand)]">Fase 4 • Financeiro</p><h1 className="text-2xl font-bold">{mode==="flow"?"Fluxo de caixa":"Relatórios financeiros"}</h1><p className="mt-1 text-sm text-[var(--ink-muted)]">{mode==="flow"?"Visão realizada e consolidada por filial.":"Receita, custo e lucro bruto por ordem de serviço."}</p></div><button onClick={()=>void load()} className="inline-flex h-10 items-center gap-2 border border-[var(--line)] px-4 text-sm font-semibold"><RefreshCw size={16}/>Atualizar</button></div>
