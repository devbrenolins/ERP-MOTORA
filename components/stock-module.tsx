...
import { AlertTriangle, ArrowDownToLine, ArrowUpFromLine, Boxes, LoaderCircle, RefreshCw, Search, X } from "lucide-react";
...
export function StockModule() {
  ...
  const filteredPositions = useMemo(() => {
    ...
    return positions.filter((item) => [item.product_name, item.sku, item.warehouse_name].some((value) => value.toLocaleLowerCase("pt-BR").includes(term)));
  }, [positions, query]);
  ...
  return <main className="mx-auto max-w-[1520px] px-4 py-5 lg:px-7 lg:py-6">
    <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div><p className="mb-1 text-[11px] font-bold uppercase tracking-[.14em] text-[var(--brand)]">Suprimentos e estoque</p><h1 className="text-2xl font-bold tracking-[-.025em]">Estoque em tempo real</h1><p className="mt-1 text-sm text-[var(--ink-muted)]">Acompanhe saldos, reservas, custo médio e todo o histórico de movimentos.</p></div>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_360px]">
      <section className="min-w-0 border border-[var(--line)] bg-[var(--surface)]">
        <div className="border-b border-[var(--line)] px-4 py-4 sm:px-5"><div className="flex items-center justify-between"><div><h2 className="font-bold">Posição por depósito</h2><p className="mt-0.5 text-xs text-[var(--ink-muted)]">Disponível = saldo físico − reservas ativas</p></div><Boxes size={19} className="text-[var(--brand)]" /></div><div className="relative mt-3 max-w-md"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--brand)]" /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} className="h-10 w-full border border-[var(--line-strong)] pl-9 pr-3 text-sm outline-none focus:border-[var(--brand)]" placeholder="Buscar produto, código ou depósito" aria-label="Buscar no estoque" /></div></div>
        {loading ? <div className="grid min-h-72 place-items-center"><LoaderCircle className="animate-spin text-[var(--brand)]" /></div> : filteredPositions.length ? <><div className="divide-y divide-[var(--line)] md:hidden">{filteredPositions.map((item) => <article key={`${item.warehouse_id}:${item.product_id}`} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{item.product_name}</p><p className="mt-1 text-xs text-[var(--ink-muted)]">{item.sku} • {item.warehouse_name}</p></div><strong className={Number(item.available) <= Number(item.minimum_stock) ? "text-[var(--danger)]" : "text-[var(--brand)]"}>{number(item.available)} {item.unit}</strong></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><span>Saldo: <b>{number(item.on_hand)}</b></span><span>Reservado: <b>{number(item.reserved)}</b></span><span>Custo: <b>{currency(item.average_cost)}</b></span><span>Comprar: <b>{Number(item.suggested_purchase) > 0 ? number(item.suggested_purchase) : "—"}</b></span></div></article>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[820px] text-left text-xs"><thead className="bg-[var(--surface-muted)] text-[10px] uppercase tracking-[.06em] text-[var(--ink-muted)]"><tr><th className="px-4 py-3">Produto</th><th className="px-4 py-3">Depósito</th><th className="px-4 py-3 text-right">Físico</th><th className="px-4 py-3 text-right">Reservado</th><th className="px-4 py-3 text-right">Disponível</th><th className="px-4 py-3 text-right">Custo médio</th><th className="px-4 py-3 text-right">Sugestão</th></tr></thead><tbody className="divide-y divide-[var(--line)]">{filteredPositions.map((item) => <tr key={`${item.warehouse_id}:${item.product_id}`} className="hover:bg-[var(--surface-muted)]"><td className="px-4 py-3"><span className="font-bold">{item.product_name}</span><span className="mt-0.5 block text-[10px] text-[var(--ink-muted)]">{item.sku}</span></td><td className="px-4 py-3">{item.warehouse_name}</td><td className="px-4 py-3 text-right">{number(item.on_hand)} {item.unit}</td><td className="px-4 py-3 text-right">{number(item.reserved)}</td><td className={`px-4 py-3 text-right font-bold ${Number(item.available) <= Number(item.minimum_stock) ? "text-[var(--danger)]" : "text-[var(--brand)]"}`}>{number(item.available)}</td><td className="px-4 py-3 text-right">{currency(item.average_cost)}</td><td className="px-4 py-3 text-right">{Number(item.suggested_purchase) > 0 ? number(item.suggested_purchase) : "—"}</td></tr>)}</tbody></table></div></> : <div className="grid min-h-72 place-items-center px-6 text-center"><div><Boxes className="mx-auto text-[var(--ink-muted)]" /><p className="mt-3 text-sm font-bold">{query ? "Nenhum item encontrado" : "Nenhum saldo registrado"}</p><p className="mt-1 text-xs text-[var(--ink-muted)]">{query ? "Tente buscar por outro nome, código ou depósito." : "Cadastre um produto e registre a primeira entrada."}</p></div></div>}
      </section>
      <form onSubmit={submit} className="h-fit border border-[var(--line)] bg-[var(--surface)]">
        ...
        <div className="space-y-4 p-5"><SearchableSelect label="Produto" value={form.product_id} required options={products} onChange={(value) => setForm({ ...form, product_id: value })} /><SearchableSelect label="Depósito" value={form.warehouse_id} required options={warehouses} onChange={(value) => setForm({ ...form, warehouse_id: value })} /><label className="block text-xs font-bold">Tipo<select value={form.movement_type} onChange={(event) => setForm({ ...form, movement_type: event.target.value })} className="mt-2 h-10 w-full border border-[var(--line-strong)] bg-white px-3 text-sm">{movementOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><div className="grid grid-cols-2 gap-3"><Input label="Quantidade" type="number" value={form.quantity} required onChange={(value) => setForm({ ...form, quantity: value })} /><Input label="Custo unitário" type="number" value={form.unit_cost} onChange={(value) => setForm({ ...form, unit_cost: value })} /></div><label className="block text-xs font-bold">Observação<textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="mt-2 h-20 w-full border border-[var(--line-strong)] p-3 text-sm" /></label><button disabled={saving || !form.product_id || !form.warehouse_id} className="inline-flex h-10 w-full items-center justify-center gap-2 bg-[var(--brand)] px-4 text-sm font-bold text-white disabled:opacity-50">{saving ? <LoaderCircle size={16} className="animate-spin" /> : form.movement_type.endsWith("in") || form.movement_type === "customer_return" ? <ArrowDownToLine size={16} /> : <ArrowUpFromLine size={16} />}Registrar movimentação</button></div>
      </form>
    </div>
