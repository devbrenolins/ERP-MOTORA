...
import { CalendarDays, CarFront, ClipboardCheck, FileText, Plus, ShieldCheck, Users, Wrench } from "lucide-react";
...
type Metrics = { customers: number | null; appointments: number | null; workOrders: number | null; estimates: number | null };

export function ErpShell() {
  const [metrics, setMetrics] = useState<Metrics>({ customers: null, appointments: null, workOrders: null, estimates: null });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const [customers, appointments, orders, estimates] = await Promise.all([
        ...
        setMetrics({ customers: customers.count ?? 0, appointments: appointments.count ?? 0, workOrders: orders.count ?? 0, estimates: estimates.count ?? 0 });
        setConnected(true);
  ...
  return <AppShell><main className="mx-auto max-w-[1520px] px-4 py-5 lg:px-7 lg:py-6">
    <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="mb-1 text-[11px] font-bold uppercase tracking-[.14em] text-[var(--brand)]">Fase 2 em operação</p><h1 className="text-2xl font-bold tracking-[-.025em]">Visão geral da oficina</h1><p className="mt-1 text-sm text-[var(--ink-muted)]">Acompanhe o fluxo do atendimento até a ordem de serviço.</p></div><Link href="/recepcao" className="inline-flex h-10 items-center justify-center gap-2 bg-[var(--brand)] px-4 text-sm font-bold text-white"><Plus size={17} />Novo check-in</Link></div>

    {!connected && <div className="mb-5 border border-[#d9c48c] bg-[#fff9e8] px-4 py-3 text-xs text-[#7b5a00]"><strong>Ambiente de demonstração:</strong> conecte o Supabase e aplique as duas migrations para carregar indicadores e persistir os registros.</div>}

    <section className="grid gap-px border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2 xl:grid-cols-4">{cards.map(([label, value, helper]) => <article key={label} className="bg-[var(--surface)] p-5"><p className="text-xs font-semibold text-[var(--ink-muted)]">{label}</p><p className="mt-3 text-2xl font-bold">{value ?? "—"}</p><p className="mt-2 text-[11px] text-[var(--ink-muted)]">{helper}</p></article>)}</section>

    <section className="mt-5 border border-[var(--line)] bg-[var(--surface)]"><div className="border-b border-[var(--line)] px-5 py-4"><h2 className="font-bold">Operação principal</h2><p className="mt-0.5 text-xs text-[var(--ink-muted)]">Módulos integrados desta fase</p></div><div className="grid sm:grid-cols-2 xl:grid-cols-4">{modules.map((item) => { const Icon = item.icon; return <Link key={item.href} href={item.href} className="group border-b border-r border-[var(--line)] p-5 transition hover:bg-[var(--surface-muted)]"><div className="grid size-9 place-items-center bg-[var(--brand-soft)] text-[var(--brand)]"><Icon size={18} /></div><h3 className="mt-4 text-sm font-bold group-hover:text-[var(--brand)]">{item.label}</h3><p className="mt-1 text-xs text-[var(--ink-muted)]">{item.description}</p></Link>; })}</div></section>
  </main></AppShell>;
}
