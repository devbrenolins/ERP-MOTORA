...
export function LoginForm() {
  ...
  return (
    <form className="mt-8 space-y-5" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div><div className="mb-2 flex items-center justify-between"><label className="text-xs font-bold" htmlFor="password">Senha</label><Link href="/login?mode=recover" className="text-xs font-semibold text-[var(--brand)] hover:underline">Recuperar senha</Link></div><div className="relative"><input id="password" type={showPassword ? "text" : "password"} autoComplete="current-password" className="h-11 w-full border border-[var(--line-strong)] bg-[var(--surface)] px-3 pr-11 outline-none focus:border-[var(--brand)]" {...register("password")} /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 grid w-11 place-items-center text-[var(--ink-muted)]" aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div>{errors.password && <p className="mt-1.5 text-xs text-[var(--danger)]">{errors.password.message}</p>}</div>
